...
parser.add_argument(
    "--dataset", type=str, default="./data/dataset/", help="dataset path"
)
parser.add_argument(
    "--video", type=str, default="./data/input.mp4", help="input video path"
)
parser.add_argument(
    "--labelmap", type=str, default="./data/labelmap.txt", help="labelmap path"
)
parser.add_argument(
    "--img-size",
    nargs="+",
    type=int,
    default=640,
    help="train image sizes, always a square",
)
parser.add_argument(
    "--tracker", type=str, default="csrt", help="name of opencv tracker for labeling"
)
...
is_dataset_paths = os.path.exists(opt.dataset + "images/") or os.path.exists(
    opt.dataset + "labels/"
)
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        for bb in tracker_arr:
            if contains((x, y), bb):
                ...
                # Stupid fix to make index function work with numpys
                i = [list(x) for x in tracker_arr].index(list(bb))
                tracker_arr.pop(i)  # remove clicked tracker from new array
                labels.pop(i)
...
frame_num = (
    max([int(name.split(".")[0]) for name in os.listdir(opt.dataset + "images/")])
    if use_last
    else 0
)
play = True
