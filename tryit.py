...
parser.add_argument('--model', type=str, default='./data/model/model.tflite', help='model path')
...
def load_pbtxt(pbtxt_path):
	...
	if os.path.isfile(pbtxt_path):
		with open(pbtxt_path, 'r') as lm:
			labels = [line.split("'")[1] for line in lm.readlines() if "'" in line] # kind of bad but should work :) (I do not know what's better)
	return labels
